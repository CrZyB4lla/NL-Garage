...
ESX.RegisterServerCallback('apoysus_garage:loadVehicles', function(source, cb)
	local ownedCars = {}
	local s = source
...
ESX.RegisterServerCallback('apoysus_garage:loadVehicle', function(source, cb, plate)
...
ESX.RegisterServerCallback('apoysus_garage:isOwned', function(source, cb, plate)
...
RegisterNetEvent('apoysus_garage:changeState')
AddEventHandler('apoysus_garage:changeState', function(plate, state)
	MySQL.Sync.execute("UPDATE owned_vehicles SET `stored` = @state WHERE `plate` = @plate", {['@state'] = state, ['@plate'] = plate})
end)

RegisterNetEvent('apoysus_garage:saveProps')
AddEventHandler('apoysus_garage:saveProps', function(plate, props)
