...
local garages = {
	{vector3(100.98, -1073.41, 29.22), vector3(104.31, -1078.69, 29.02), 339.77 },
    {vector3(-1519.98, 80.26, 56.73), vector3(-1531.92, 83.11, 56.74), 323.64 },
    {vector3(-281.171, -888.600, 31.318), vector3(-277.853, -898.670, 30.903), 340.39 },
    {vector3(-348.975, -874.027, 31.318), vector3(-357.720, -881.208, 30.894), 354.33 },
...
local coordinate = {
{ 100.98, -1073.41, 29.22, nil, 69.66, nil, -100858228},
...
{-281.171, -888.600, 31.318, nil, 335.08, nil, -1806291497},
{-348.975, -874.027, 31.318, nil, 187.60, nil, -1806291497},
...
RegisterNUICallback('enable-parkout', function(data, cb)
    	ESX.TriggerServerCallback('apoysus_garage:loadVehicles', function(ownedCars)
		if #ownedCars == 0 then
...
RegisterNUICallback('enable-parking', function(data, cb)
    ...
    for key, value in pairs(vehicles) do
        ESX.TriggerServerCallback('apoysus_garage:isOwned', function(owned)
...
RegisterNUICallback('park-out', function(data, cb)
    ...
    ESX.TriggerServerCallback('apoysus_garage:loadVehicle', function(vehicle)
    ...
    TriggerServerEvent('apoysus_garage:changeState', data.plate, 0)
...
RegisterNUICallback('park-in', function(data, cb)
    ...
    for key, value in pairs(vehicles) do
        if GetVehicleNumberPlateText(value) == data.plate then
            TriggerServerEvent('apoysus_garage:saveProps', data.plate, ESX.Game.GetVehicleProperties(value))
            TriggerServerEvent('apoysus_garage:changeState', data.plate, 1)
